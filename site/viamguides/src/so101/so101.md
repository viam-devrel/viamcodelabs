...
1. Under the `MoveToPosition` panel, you can test the end effector movement by adjusting pose values. To move the arm up, increase the `Z` value and press **Execute**. To move it down, decrease the `Z` value. To rotate the wrist, modify the `theta` value in the orientation field.
